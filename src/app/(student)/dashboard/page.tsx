...
export default async function StudentDashboard() {
  ...
  return (
    <div className="space-y-8">
      {/* Header - Actions only */}
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-end border-b border-slate-200 pb-6">
        <div>
          {/* Titre déplacé dans le layout */}
        </div>

        {!hasCurrentApplication && (
          <Link
            href="/application/new"
            className="inline-flex items-center gap-2 rounded-sm bg-blue-900 px-4 py-2 font-semibold text-white hover:bg-blue-800 transition-colors shadow-sm"
          >
      ...
      {currentApplication ? (
        <div className="rounded-sm border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-start justify-between">
            ...
            <Link
              ...
              className="rounded-sm bg-slate-50 border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100 transition-colors"
            >
              Voir le dossier
            </Link>
          </div>
        ...
        <div className="rounded-sm border-2 border-dashed border-slate-300 p-12 text-center">
          ...
          <Link
            href="/application/new"
            className="mt-6 inline-flex items-center gap-2 rounded-sm bg-blue-900 px-4 py-2 font-semibold text-white hover:bg-blue-800"
          >
            <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
            </svg>
      ...
      {applications && applications.length > 1 && (
        <div>
          <h2 className="mb-4 text-lg font-semibold text-blue-900 font-serif">Historique</h2>
      ...
      <div className="rounded-sm border border-slate-200 bg-white p-6">
        <h3 className="font-semibold text-blue-900 font-serif">Informations</h3>
        <dl className="mt-4 grid gap-4 sm:grid-cols-2">
          <div>
            <dt className="text-sm text-slate-500">Majeure</dt>
